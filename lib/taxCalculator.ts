...
// ---------- Types ----------
import { getPrincipalFromMensualite, generateAmortizationSchedule, getAnnualAmortization } from './calculations/loan';
import { calcElectricityCostST, calcWaterCostST } from './calculations/dpe';
import { DEFAULT_CHARGES } from './calculations/defaultCharges';
...
export interface PropertyInputs {
  ...
  codePostal: string;
  surface: number;            // m²
  propertyType: "T1/T2" | "T3+" | "Maison" | "Autre";
  dpe: "A" | "B" | "C" | "D" | "E" | "F" | "G";
...
export interface ScenarioResult {
  ...
  vacanceLocativeAnnuelle: number;  // LLD : manque à gagner vacance (0 pour CD/Mixte)
  utilitesAnnuels: number;          // CD/Mixte : eau + électricité (0 pour LLD)
  consommablesAnnuels: number;      // CD/Mixte : kit d'accueil (0 pour LLD)
  // Mixte uniquement : ventilation des revenus LD / CD
  mixteRevenuLD?: number;
  mixteRevenuCD?: number;
  opportunityCost?: {
...
export interface AnalysisResult {
  ...
  amortizationYearly: import('./calculations/loan').AmortizationYear[];
}
...
/**
 * Coefficient de prix dynamique basé sur le taux d'occupation du mois.
 *
 * Règle Yield Management :
 *   TO < 60%          → prix × 0.8  (-20%)
 *   60% ≤ TO < 68%    → prix × 0.9  (-10%)
 *   68% ≤ TO ≤ 80%    → prix × 1.0  (prix normal)
 *   TO > 80%          → prix × 1.1  (+10%)
 */
export function getDynamicPriceMultiplier(occupancyRate: number): number {
  const toPct = occupancyRate * 100; // Convertir 0-1 en 0-100
  if (toPct < 60) return 0.8;
  if (toPct < 68) return 0.9;
  if (toPct <= 80) return 1.0;
  return 1.1;
}

// Mapping noms français complets → indice 0-11 dans la matrice de saisonnalité
const MONTH_FR_INDEX: Record<string, number> = {
...
/**
 * Répartit les jours d'usage personnel sur les mois sélectionnés.
 * Si `selectedMonths` est fourni et non vide, les jours sont distribués
 * sur ces mois (du dernier au premier pour remplir les mois les plus rentables
 * en priorité). Sinon, fallback sur les mois d'été (Août > Juillet > Juin).
 * Retourne un tableau de 12 éléments : le nombre de jours personnels par mois.
 */
function distributePersonalUseDays(
  seasonality: SeasonalityMonth[],
  totalPersonalDays: number,
  selectedMonths?: string[]
): number[] {
  const personalDaysPerMonth = new Array(12).fill(0);

  // Cible : mois sélectionnés par l'utilisateur, ou été par défaut
  let targetIndices: number[];
  if (selectedMonths && selectedMonths.length > 0) {
    targetIndices = selectedMonths
      .map(m => MONTH_FR_INDEX[m] ?? -1)
      .filter(i => i >= 0)
      .sort((a, b) => b - a); // du plus tardif au plus précoce
  } else {
    targetIndices = [7, 6, 5]; // Août, Juillet, Juin
  }

  const maxDays = targetIndices.reduce((s, idx) => s + seasonality[idx].days, 0);
  let remaining = Math.min(totalPersonalDays, maxDays);

  for (const idx of targetIndices) {
    if (remaining <= 0) break;
    const taken = Math.min(remaining, seasonality[idx].days);
    personalDaysPerMonth[idx] = taken;
    remaining -= taken;
  }

  return personalDaysPerMonth;
}

/**
 * Calcul du revenu Courte Durée avec Yield Management mensuel.
 *
 * Pour chaque mois :
 *   1. Calcul des jours disponibles = jours du mois − jours usage perso
 *   2. Nuits louées = jours disponibles × taux d'occupation
 *   3. Prix dynamique = ADR de base × getDynamicPriceMultiplier(TO du mois)
 *   4. Revenu du mois = nuits louées × prix dynamique
 *
 * @param baseADR            Prix de nuit de base (Average Daily Rate)
 * @param seasonality        Matrice de saisonnalité (12 mois)
 * @param personalUseDays    Nombre total de jours d'usage personnel
 * @param onlySummer         Si true, ne calcule que les mois d'été (pour le Mixte)
 * @returns Objet avec revenu total, nuitées effectives, et détail par mois
 */
export function calculateMonthlyYieldRevenue(
  baseADR: number,
  seasonality: SeasonalityMonth[],
  personalUseDays: number = 0,
  onlySummer: boolean = false,
  personalUseMonths?: string[]
): {
  revenuAnnuel: number;
  nuiteesEffectives: number;
  detailMensuel: Array<{
    month: string;
    joursDisponibles: number;
    nuiteesLouees: number;
    prixDynamique: number;
    multiplier: number;
    revenu: number;
  }>;
  lostRevenue: number;
} {
  const personalDaysPerMonth = distributePersonalUseDays(seasonality, personalUseDays, personalUseMonths);
  ...
  for (let i = 0; i < 12; i++) {
    const month = seasonality[i];

    // Filtre : si onlySummer, ne calculer que les mois d'été
    if (onlySummer && !month.isSummer) {
      detail.push({
        month: month.name,
        joursDisponibles: 0,
        nuiteesLouees: 0,
    ...
    // 1. Jours disponibles après déduction de l'usage personnel
    const joursDisponibles = Math.max(0, month.days - personalDaysPerMonth[i]);

    // 2. Nuitées louées = jours disponibles × taux d'occupation
    const nuiteesLouees = joursDisponibles * month.occupancyRate;

    // 3. Prix dynamique = ADR × coefficient yield management
    const multiplier = getDynamicPriceMultiplier(month.occupancyRate);
    const prixDynamique = baseADR * multiplier;

    // 4. Revenu du mois
    const revenuMois = nuiteesLouees * prixDynamique;

    // Calcul du manque à gagner (jours perso × prix dynamique × TO)
    if (personalDaysPerMonth[i] > 0) {
      lostRevenue += personalDaysPerMonth[i] * month.occupancyRate * prixDynamique;
    }

    revenuTotal += revenuMois;
    nuiteesTotal += nuiteesLouees;
  ...
  return {
    ...
    nuiteesEffectives: nuiteesTotal,
    detailMensuel: detail,
    lostRevenue,
  };
}
...
export function runAnalysis(
  ...
  // Prix Total d'Acquisition = bien + frais notaire (~8%) + travaux
  const prixTotalAcquisition = valeurBien * 1.08 + inputs.renovationBudget;
  ...
  // Total annual charges (copro + taxe foncière + PNO + internet)
  const chargesAnnuelles =
    inputs.chargesCopropriete + inputs.taxeFonciere + inputs.assurancePNO +
    DEFAULT_CHARGES.internetMensuel * 12;

  // Règle C : Calcul précis des intérêts avec le tableau d'amortissement
  let interetsDeductibles = 0;
  let annualAmortization: import('./calculations/loan').AmortizationYear[] = [];

  if (inputs.hasOngoingLoan) {
    const principal = getPrincipalFromMensualite(inputs.mensualiteCredit, inputs.interestRate, inputs.dureeCredit);
    const schedule = generateAmortizationSchedule(principal, inputs.interestRate, inputs.dureeCredit);
    annualAmortization = getAnnualAmortization(schedule);
    
    // Année 1 pour le calcul fiscal immédiat
    if (annualAmortization.length > 0) {
      interetsDeductibles = annualAmortization[0].interetsPayes;
    }
  }
  ...
  // ─────────────────────────────────────────────────────────────────
  // A. CALCULS COMMUNS (Revenus Bruts)
  // ─────────────────────────────────────────────────────────────────
  
  // Choose exact Ministry data if available based on selected propertyType
  let loyerM2LD = marketLong.loyerMoyenM2;
  if (marketLong.donneesMinistere) {
    if (inputs.propertyType === "T1/T2" && marketLong.donneesMinistere.app12) {
      loyerM2LD = marketLong.donneesMinistere.app12;
    } else if (inputs.propertyType === "T3+" && marketLong.donneesMinistere.app3) {
      loyerM2LD = marketLong.donneesMinistere.app3;
    } else if (inputs.propertyType === "Maison" && marketLong.donneesMinistere.mai) {
      loyerM2LD = marketLong.donneesMinistere.mai;
    } else if (marketLong.donneesMinistere.app) {
      loyerM2LD = marketLong.donneesMinistere.app;
    }
  }

  // Loyer brut (pré-vacance) pour affichage — vacance appliquée au loyer effectif
  const loyerLDMensuelBrut = Math.min(
    loyerM2LD * inputs.surface,
    inputs.surface * (marketLong.plafondLoyer ?? Infinity)
  );
  const loyerLDMensuel = loyerLDMensuelBrut * (1 - DEFAULT_CHARGES.vacanceLocativeLld);
  ...
  // Rendement brut : loyer théorique pré-vacance / prix acquisition
  // Rendement net : cashflow annuel net (après charges + crédit + fiscalité) / prix acquisition
  const rendementBrutLD =
    prixTotalAcquisition > 0 ? (loyerLDMensuelBrut * 12 / prixTotalAcquisition) * 100 : 0;
  const rendementNetLD =
    prixTotalAcquisition > 0
      ? (cashflowLDMensuel * 12 / prixTotalAcquisition) * 100
      : 0;
  ...
  const scenarioLD: ScenarioResult = {
    ...
    revenuBrutAnnuel: bloqueDPE ? 0 : loyerLDMensuelBrut * 12,
    revenuBrutMensuel: bloqueDPE ? 0 : loyerLDMensuelBrut,
    chargesAnnuelles,
    ...
    alerteDPE,
    legalAlerts: [],
    vacanceLocativeAnnuelle: bloqueDPE ? 0 : loyerLDMensuelBrut * 12 * DEFAULT_CHARGES.vacanceLocativeLld,
    utilitesAnnuels: 0,
  ...
  const personalUseMonths = options?.fees?.personalUseMonths;
  const yieldResult = calculateMonthlyYieldRevenue(
    marketShort.adr,
    seasonalityMatrix,
    personalDays,
    false, // tous les mois pour la courte durée
    personalUseMonths
  );

  let revenuCDAnnuel = yieldResult.revenuAnnuel;
  let nuiteesEffectives = yieldResult.nuiteesEffectives;
  let lostRevenue = yieldResult.lostRevenue;
  ...
  if (isResidencePrincipale && nuiteesEffectives > 120) {
    ...
    revenuCDAnnuel = revenuCDAnnuel * ratio120;
  }
  ...
  // Eau + électricité : à la charge du propriétaire en courte durée (inclus dans le prix de la nuit)
  const utilitesCD = shortTermBlocked ? 0 :
    calcElectricityCostST(inputs.surface, inputs.dpe) +
    calcWaterCostST(nuiteesEffectives);
  const consommablesCD = shortTermBlocked ? 0 : DEFAULT_CHARGES.consommablesLcdMensuel * 12;
  const chargesCD = chargesAnnuelles + fraisConciergerie + fraisMenage + utilitesCD + consommablesCD;
  const revenuCDNet = revenuCDAnnuel - fraisConciergerie - fraisMenage - utilitesCD - consommablesCD;
  ...
  const rendementBrutCD =
    prixTotalAcquisition > 0 ? (revenuCDAnnuel / prixTotalAcquisition) * 100 : 0;
  const rendementNetCD =
    prixTotalAcquisition > 0 && !shortTermBlocked
      ? (cashflowCDMensuel * 12 / prixTotalAcquisition) * 100
      : 0;
  ...
  const scenarioCD: ScenarioResult = {
    ...
    opportunityCost,
    vacanceLocativeAnnuelle: 0,
    utilitesAnnuels: shortTermBlocked ? 0 : utilitesCD,
    consommablesAnnuels: shortTermBlocked ? 0 : consommablesCD,
  };

  // ====== SCENARIO C: Mixte (9 mois LD + 3 mois CD été) — Yield Management ======
  const moisLD = 9;
  const moisCD = 3;
  const revenuMixteLD = loyerLDMensuel * moisLD;

  // Yield Management : calcul dynamique pour les 3 mois d'été uniquement
  // Les jours d'usage perso sont déduits des mois d'été avant calcul
  const yieldMixteEte = calculateMonthlyYieldRevenue(
    marketShort.adr,
    seasonalityMatrix,
    personalDays, // les jours perso impactent directement les mois d'été
    true // onlySummer = true → ne calcule que Juin, Juillet, Août
  );

  let revenuMixteCD = yieldMixteEte.revenuAnnuel;
  let nuiteesEte = yieldMixteEte.nuiteesEffectives;
  ...
  // Électricité proratisée 3/12 ; eau calculée sur les nuits d'été réellement occupées
  const utilitiesMixte = shortTermBlocked ? 0 :
    calcElectricityCostST(inputs.surface, inputs.dpe, 3 / 12) +
    calcWaterCostST(nuiteesEte);
  const consommablesMixte = shortTermBlocked ? 0 : DEFAULT_CHARGES.consommablesLcdMensuel * 3;
  const revenuMixteTotal = revenuMixteLD + revenuMixteCD - fraisConciergerieM - fraisMenageM - utilitiesMixte - consommablesMixte;
  const chargesMixteSpecifiques = fraisConciergerieM + fraisMenageM + utilitiesMixte + consommablesMixte;
  ...
  const rendementBrutMixte =
    prixTotalAcquisition > 0
      ? ((revenuMixteLD + revenuMixteCD) / prixTotalAcquisition) * 100
      : 0;
  const rendementNetMixte =
    prixTotalAcquisition > 0
      ? (cashflowMixteMensuel * 12 / prixTotalAcquisition) * 100
      : 0;
  ...
  const scenarioMixte: ScenarioResult = {
    ...
    vacanceLocativeAnnuelle: bloqueDPE ? 0 : loyerLDMensuelBrut * moisLD * DEFAULT_CHARGES.vacanceLocativeLld,
    utilitesAnnuels: shortTermBlocked ? 0 : utilitiesMixte,
    consommablesAnnuels: shortTermBlocked ? 0 : consommablesMixte,
    mixteRevenuLD: revenuMixteLD,
    mixteRevenuCD: shortTermBlocked ? 0 : revenuMixteCD,
  };
  ...
  return {
    ...
    partsFiscales: parts,
    quotientFamilial,
    amortizationYearly: annualAmortization,
    recommendation,
  };
}
...
export interface ShortTermFees {
  ...
  /** Mois sélectionnés pour l'usage personnel (noms français complets) */
  personalUseMonths?: string[];
}
